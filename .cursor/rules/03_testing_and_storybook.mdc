...
**Stories:** `Meta<typeof Component>`, `StoryObj<typeof Component>`, `tags: ['autodocs']`, `title: 'Atom/ComponentName'` (or `Molecule/...` / `Organism/...` per atomic design). Use Atom for primitives (Button, Card), Molecule for composed units (ProductCard), Organism for sections (Header, Hero, ProductSuite). Provide `Default` and variant stories. Addons: a11y, docs.
